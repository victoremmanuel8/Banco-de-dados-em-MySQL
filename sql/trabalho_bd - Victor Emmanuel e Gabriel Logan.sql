...
update tb_funcionario
	join tb_cargo on (
     tb_cargo.cd_cargo = tb_funcionario.cd_cargo and
    tb_funcionario.cd_funcionario = 2 
    )
    join tb_setor on (tb_setor.cd_setor = tb_funcionario.cd_setorFuncionario)
    set
		tb_funcionario.nm_funcionario = 'Marisa Goes',
        tb_cargo.ds_cargo = 'Administra e faz manutenções, demissões e equipes no geral',
        tb_setor.nm_setor = 'Recursos Humanos'
	where
		tb_funcionario.cd_funcionario = 2 and
        tb_setor.cd_setor = 4;
